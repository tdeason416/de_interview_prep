...
-- Duplicates in pairings can be left in the solution. For example, (2, 3) and (3, 2) should both be in the soluti
...
with ranked as (
    SELECT
        employee_id,
        RANK() OVER(ORDER BY popularity DESC, employee_id) as descending,
        RANK() OVER(ORDER BY popularity, employee_id DESC) as ascending,
        popularity
    FROM
        facebook_hack_survey
)
...
SELECT
    a.employee_id employee_fan_id,
    b.employee_id employee_opposition_id,
    a.popularity,
    b.popularity
FROM ranked a
JOIN ranked b
    ON a.ascending = b.descending